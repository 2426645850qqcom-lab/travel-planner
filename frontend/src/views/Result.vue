  ...
  <div class="result-container">
    ...
    <div v-else class="content">
      ...
      <el-card class="map-card" shadow="hover">
        ...
        <div v-if="mapLoadError" class="static-map-fallback">
          <div class="map-placeholder">
            <el-icon :size="48" color="#909399"><MapLocation /></el-icon>
            <p class="map-placeholder-title">地图加载失败</p>
            <p class="map-placeholder-desc">
              当前城市：{{ tripPlan?.city }}<br/>
              请确认高德 Key 已开启 <strong>Web端(JS API)</strong> 权限，并将 <code>localhost</code> 加入域名白名单。
            </p>
          </div>
        </div>
        <div v-else id="map-container" class="map-container"></div>
      </el-card>
...
// 静态地图 URL（AMap 加载失败时作为回退）
function updateStaticMapUrl() {
  if (tripPlan.value?.city) {
    staticMapUrl.value = getStaticMapUrl(tripPlan.value.city)
  }
}
...
async function initMap() {
  ...
  const loadTimeout = setTimeout(() => {
    if (!mapLoaded.value) {
      console.warn('[Map] 高德地图加载超时，切换到静态地图')
      mapLoadError.value = true
    }
  }, 8000)
    ...
    AMap.plugin(['AMap.Geocoder'], function() {
      ...
      geocoder.getLocation(city, (status: string, result: any) => {
        ...
        try {
          ...
          tripPlan.value!.days?.forEach((day: any) => {
          ...
          console.error('[Map] 创建地图实例失败:', mapError)
          mapLoadError.value = true
        }
      })
    })
  } catch (error) {
    console.error('[Map] 地图创建失败:', error)
    mapLoadError.value = true
  }
}
